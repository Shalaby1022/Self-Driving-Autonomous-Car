...
from Plugins.testpilot.testpilot.utils.Utils import _process_image
...
def main():
    ...
    try:
        ...
        m3 = [x for x in blueprints if x.tags[2] == 'model3'] #العربية اللى بنتها
        ...
        while img_counter<10_000:
            ...
            if distance_progress>5:
                ...
                cv2.imwrite('F:/SelfDrive/Semantic segmentation/out_sem/rgb/%06d.png' % time_grab, rgb_im)
                cv2.imwrite('f:/SelfDrive/Semantic segmentation/out_sem/sem/%06d.png' % time_grab, sem_im)
                previous_position = hero_car.get_transform()
...
population = [NeuralNetwork(input_size=3, hidden_size=8, output_size=2) for _ in range(population_size)]

# Carla Simulator setup
try:
    client = carla.Client('localhost', 2000)
    client.set_timeout(10.0)  # Set timeout for client connection

    world = client.load_world('Town01')  # Load the desired map by name
    world.set_weather(carla.WeatherParameters.ClearNoon)

    blueprint_library = world.get_blueprint_library()
    
    # Define a fixed spawn point for the vehicle
    spawn_point_vehicle = carla.Transform(carla.Location(x=100, y=100, z=0), carla.Rotation(yaw=0))

    # Spawn a vehicle for the neural network to control
    vehicle_bp = random.choice(blueprint_library.filter('vehicle.*.*'))
    vehicle = world.spawn_actor(vehicle_bp, spawn_point_vehicle)
    vehicle.set_autopilot(True)
    
    # Spawn a camera sensor on the vehicle
    camera_bp = blueprint_library.find('sensor.camera.rgb')
    camera_transform = carla.Transform(carla.Location(x=1.5, z=2.4))
    camera = world.spawn_actor(camera_bp, camera_transform, attach_to=vehicle)
    camera.listen(lambda image: _process_image(image))

except Exception as e:
    print(f"Error: {e}")

# Function to evaluate fitness of each neural network
def evaluate_fitness(network):
    # Dummy fitness evaluation, replace with actual implementation
...
for generation in range(100):
    ...
    top_performers_indices = np.argsort(fitness_scores)[-int(population_size * 0.2):]
    ...
    for _ in range(population_size):
        ...
        child = NeuralNetwork(input_size=3, hidden_size=8, output_size=2)
        
        for i in range(child.weights_input_hidden.shape[0]):
            for j in range(child.weights_input_hidden.shape[1]):
                if random.random() < 0.5:
                    child.weights_input_hidden[i][j] = parent1.weights_input_hidden[i][j]
                else:
                    child.weights_input_hidden[i][j] = parent2.weights_input_hidden[i][j]
        
        for i in range(child.weights_hidden_output.shape[0]):
            for j in range(child.weights_hidden_output.shape[1]):
                if random.random() < 0.5:
                    child.weights_hidden_output[i][j] = parent1.weights_hidden_output[i][j]
                else:
        ...
        for i in range(child.weights_input_hidden.shape[0]):
            for j in range(child.weights_input_hidden.shape[1]):
                if random.random() < mutation_rate:
                    child.weights_input_hidden[i][j] += np.random.randn() * 0.1
        
        for i in range(child.weights_hidden_output.shape[0]):
            for j in range(child.weights_hidden_output.shape[1]):
                if random.random() < mutation_rate:
                    child.weights_hidden_output[i][j] += np.random.randn() * 0.1
